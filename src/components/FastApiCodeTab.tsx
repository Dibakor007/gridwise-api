...
import { Copy, Check, FileCode, Terminal, CheckCircle2 } from 'lucide-react';

const REQUIREMENTS_TXT = `fastapi==0.115.8
...
export function FastApiCodeTab() {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="text-base font-bold text-white">FastAPI & PuLP Source Files</h2>
        </div>
        <p className="text-xs text-slate-300 leading-relaxed">
          The complete, self-contained Python implementation using <strong>FastAPI</strong>, <strong>Pydantic v2</strong>, <strong>Google GenAI SDK (Gemini 2.5 Flash)</strong>, and <strong>PuLP Linear Programming</strong>. Saved in the project root as <code className="text-cyan-300 font-mono">main.py</code> and <code className="text-cyan-300 font-mono">requirements.txt</code>.
        </p>
        ...
        <div className="mt-4 p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between text-xs font-mono">
          <span className="text-slate-400">Quick run command:</span>
          <code className="text-emerald-400">pip install -r requirements.txt && uvicorn main:app --port 8000</code>
        </div>
      </div>
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-white font-mono">requirements.txt</span>
          </div>
          <button
            onClick={() => copyToClipboard(REQUIREMENTS_TXT, setCopiedReqs)}
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-white font-mono">main.py (Complete Production Code)</span>
          </div>
          <button
            onClick={() => copyToClipboard(MAIN_PY_CODE, setCopiedMain)}
            className="flex items-center gap-1.5 px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs rounded-lg transition-colors"
          >
        ...
        <pre className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-mono text-slate-300 overflow-x-auto max-h-[600px] overflow-y-auto">
          {MAIN_PY_CODE}
        </pre>
      </div>
    </div>
